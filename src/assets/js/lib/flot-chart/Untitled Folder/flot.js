...
(function (root, factory) {
  if (typeof define === 'function' && define.amd) {
    ...
    define(['dom-tools', 'EventEmitter'], function (DOMTools, EventEmitter) {
      return (root.Plot = factory(DOMTools, EventEmitter));
    });
  } else if (typeof module === 'object' && module.exports) {
    // Node. Does not work with strict CommonJS, but only CommonJS-like
    // environments that support module.exports, like Node.
    module.exports = factory(require('dom-tools'), require('wolfy87-eventemitter'));
  } else {
    // Browser globals
    root.Plot = factory(root.DOMTools, root.EventEmitter);
  }
}(this, function (DOMTools, EventEmitter) {
  ...
  var Color = (function() {
    ...
    color.extract = function (/* Element */ elem, css) {
      ...
      do {
        c = DOMTools.css(elem, css).toLowerCase();
        // keep going until we find an element that has color, or
        // we hit the body or root (have no parent)
        if (c != '' && c != 'transparent')
          break;
        elem = DOMTools.parent(elem);
      } while (elem != null && !DOMTools.nodeName(elem.get(0), "body"));
  ...
  function Canvas(cls, container) {
    ...
    var element, el, i;
    var children = DOMTools.children(container);

    for (i = 0; i < children.length; i++) {
      el = children[i];
      if (DOMTools.hasClass(el, cls)) {
        element = el;
        break;
      }
    }
    ...
    if (element == null) {
      ...
      DOMTools.append(container, element);
    ...
    this.resize(DOMTools.width(container), DOMTools.height(container));
                ...
                for (var i = 0, position; position = positions[i]; i++) {
                  if (position.active) {
                    if (!position.rendered) {
                      DOMTools.append(layer, position.element);
                      position.rendered = true;
                    }
                  } else {
                    positions.splice(i--, 1);
                    if (position.rendered) {
                      DOMTools.remove(position.element);
  ...
  Canvas.prototype.getTextLayer = function(classes) {
    ...
    if (layer == null) {
      ...
      if (this.textContainer == null) {
        ...
        DOMTools.insertAfter(this.textContainer, this.element);
      }
      ...
      DOMTools.append(this.textContainer, layer);
  ...
  Canvas.prototype.getTextInfo = function(layer, text, font, angle, width) {
    ...
    if (info == null) {
      ...
      DOMTools.append(textLayer, element);
      ...
      if (typeof font === "object") {
        ...
        font.split(' ').forEach(function(cls) {
          DOMTools.addClass(element, cls);
        });
      }

      info = styleCache[text] = {
        width: DOMTools.outerWidth(element, true),
        height: DOMTools.outerHeight(element, true),
        element: element,
        positions: []
      };

      //element.detach();
      DOMTools.remove(element);
    }
  ...
  Canvas.prototype.addText = function(layer, x, y, text, font, angle, width, halign, valign) {
    ...
    position = {
      ...
      element: positions.length ? DOMTools.clone(info.element) : info.element,
      x: x,
      y: y
    };
  ...
  function Plot(placeholder, data_, options_) {
    ...
    plot.offset = function () {
      ...
      var o = DOMTools.offset(overlay.element);
    ...
    plot.destroy = function () {
      ...
      DOMTools.empty(placeholder); // placeholder.empty();
    ...
    plot.resize = function () {
      var width = DOMTools.width(placeholder);
      var height = DOMTools.height(placeholder);
    ...
    function parseOptions(opts) {
      ...
      var i, axisOptions, axisCount,
        fontSize = DOMTools.css(placeholder, "font-size"),
        fontSizeDefault = fontSize ? +fontSize.replace("px", "") : 13,
        fontDefaults = {
          style: DOMTools.css(placeholder, "font-style"),
          size: Math.round(0.8 * fontSizeDefault),
          variant: DOMTools.css(placeholder, "font-variant"),
          weight: DOMTools.css(placeholder, "font-weight"),
          family: DOMTools.css(placeholder, "font-family")
        };
    ...
    function setupCanvases() {
      ...
      //  })
      //  .remove();
      var children = DOMTools.children(placeholder);

      Array.prototype.forEach.call(children, function(el) {
        if (!DOMTools.hasClass(el, 'flot-overlay') && !DOMTools.hasClass(el, 'flot-base')) {
          DOMTools.remove(el);
        }
      });
      ...
      if (DOMTools.css(placeholder, 'position') == 'static') {
    ...
    function bindEvents() {
      ...
      if (options.grid.hoverable) {
        DOMTools.addEventListener(overlay.element, 'mousemove', onMouseMove);
        DOMTools.addEventListener(overlay.element,'mouseleave', onMouseLeave);
      }

      if (options.grid.clickable) {
        DOMTools.addEventListener(overlay.element, 'click', onClick);
      }
    ...
    function shutdown() {
      ...
      DOMTools.removeEventListener(overlay.element, "mousemove", onMouseMove);
      DOMTools.removeEventListener(overlay.element, "mouseleave", onMouseLeave);
      DOMTools.removeEventListener(overlay.element, "click", onClick);
    ...
    function insertLegend() {
      ...
      if (options.legend.container != null) {
        ...
        DOMTools.html(options.legend.container, '');
      } else {
        //placeholder.find(".legend").remove();
        var nodeList = DOMTools.find(placeholder, '.legend');
        Array.prototype.forEach.call(nodeList, function(node) {
          DOMTools.remove(node);
        });
      }
      ...
      if (options.legend.container != null) {
        ...
        DOMTools.html(options.legend.container, table);
      } else {
        var posObj = {},
          pos = "",
        ...
        DOMTools.append(placeholder, legend);
        ...
        if (options.legend.backgroundOpacity != 0.0) {
          ...
          var legendChildren = DOMTools.children(legend);
          ...
          if (legendChildren.length) {
            div.style.width = DOMTools.width(legendChildren[0]) + 'px';
            div.style.height = DOMTools.height(legendChildren[0]) + 'px';
          }
    ...
    function triggerClickHoverEvent(eventname, /* MouseEvent */ event, seriesFilter) {
      var offset = DOMTools.offset(overlay.element), //var offset = eventHolder.offset(),
